...
class HandshakeCoordinator:
    ...
    _instance: HandshakeCoordinator | None = None
    ...
    def __init__(self, context: zmq.Context | None = None):
        ...
        self._servers: dict[str, zmq.Socket | None] = {}  # subscriber_id -> server_socket
        self._running = True
        self._server_threads: dict[str, threading.Thread] = {}

    @classmethod
    def get_instance(cls, context: zmq.Context | None = None) -> HandshakeCoordinator:
    ...
    def stop_server(self, subscriber_id: str) -> None:
        ...
        if subscriber_id in self._servers:
            try:
                ...
                # Reduced timeout for faster cleanup (server should exit quickly)
        ...
        try:
            ...
            for subscriber_id in subscriber_ids:
                ...
                try:
                    ...
                    events = dict(poller.poll(int(timeout * 1000)))  # Convert to milliseconds
                    ...
                    if client in events:
                        ...
                        if response == ack:
                            ...
                            logger.warning(f"Unexpected response from '{subscriber_id}': {response!r}")
                    else:
...
"""Handshake utilities for coordinating component connections.

This module provides shared functions for handshake setup and cleanup
to reduce duplication across interface and operator classes.
"""
...
def cleanup_handshake(coordinator: HandshakeCoordinator, server_id: str) -> None:
    ...
    Args:
        coordinator: The HandshakeCoordinator instance
        server_id: The server ID to stop
    """
    try:
        coordinator.stop_server(server_id)
    except Exception as e:
        logger.warning(f"Error stopping handshake server {server_id}: {e}")
